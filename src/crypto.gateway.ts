...
@WebSocketGateway({ cors: true })
export class BTCGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    @WebSocketServer() server: Server;
    private readonly logger: Logger = new Logger(CryptoGateway.name);
    private readonly coinCapSocketUrl: string = 'wss://ws.coincap.io/prices?assets=bitcoin';
    private coinCapSocket: WebSocket;
    
    afterInit() {
        this.logger.log('(BTC) Gateway Initialized');
        this.startCoinCapSocket();
    }

    handleConnection(client: any) {
        this.logger.log(`(BTC) Client connected: ${client.id}`);
    }

    handleDisconnect(client: any) {
        this.logger.log(`(BTC) Client disconnected: ${client.id}`);
    }
    ...
    private startCoinCapSocket(){
        ...
        this.coinCapSocket.on('error ', (error) => {
            this.logger.error('(BTC) CoinCap WebSocket Error: ' , error);
        });

        this.coinCapSocket.on('close', () => {
            this.logger.warn('(BTC) CoinCap WebSocket closed, reconnecting...');
            setTimeout(() => this.startCoinCapSocket(), 1000);  // reconnect after 1 second
        });
    }
}